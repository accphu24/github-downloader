...
class KeepAliveService {
  ...
  Future<void> startKeepAlive({required String label}) => _startForReason('download', title: 'Đang tải file', text: label);

  /// Gọi khi không còn tác vụ tải nào đang chạy.
  Future<void> stopKeepAliveIfIdle() => _stopReason('download');

  /// Gọi khi bật tính năng theo dõi CI/CD nền.
  Future<void> startCiWatch({required String label}) => _startForReason('ci_watch', title: 'Đang theo dõi CI/CD', text: label);

  /// Gọi khi tắt tính năng theo dõi CI/CD nền.
  Future<void> stopCiWatch() => _stopReason('ci_watch');
}
